...
feedforward_keras(data_train, Y_train, data_test, Y_test)
"""
...
## For writing feature vector in CSV files

aligment_csv_file = open("Alignment_score.csv","w")


for question1 in data:
    new_line=""
    for ind, score in enumerate(question1):
        if ind == len(question1) -1:
           new_line+=str(score)+"\n"
        else:
           new_line+=str(score)+","
    aligment_csv_file.write(new_line)


# feedforward_keras("Alignment_score.csv",Y)
